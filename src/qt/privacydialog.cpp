...
#include "zcatthiscontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zcats ought to be enough for anybody." - Bill Gates, 2017
    ui->zcatspayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzcatsSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzcats->setEnabled(false);
        ui->pushButtonMintzcats->setToolTip(tr("zcats is currently disabled due to maintenance."));

        ui->pushButtonSpendzcats->setEnabled(false);
        ui->pushButtonSpendzcats->setToolTip(tr("zcats is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zcatspayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzcats_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zcats is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zcats...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zcats in ") +
...
void PrivacyDialog::on_pushButtonSpendzcats_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zcats is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zcats
        sendzcats();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zcats
    sendzcats();
}

void PrivacyDialog::on_pushButtonZcatthisControl_clicked()
{
    ZcatthisControlDialog* zcatsControl = new ZcatthisControlDialog(this);
    zcatsControl->setModel(walletModel);
    zcatsControl->exec();
}

void PrivacyDialog::setZcatthisControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzcatsSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzcats()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid catthis Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zcatspayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zcats is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " catthis </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zcats</b>";
    ...
    if (!ZcatthisControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZcatthisControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcatthis selector in case it was used
    ZcatthisControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zcats Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " catthis, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zcats Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zcats "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zcats "));
    ui->labelzcatsAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzcatsSyncStatus->setVisible(fShow);
}
