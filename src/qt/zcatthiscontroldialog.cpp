...
#include "ui_zcatthiscontroldialog.h"
...
std::list<std::string> ZcatthisControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZcatthisControlDialog::listMints;

ZcatthisControlDialog::ZcatthisControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZcatthisControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZcatthisControlDialog::~ZcatthisControlDialog()
{
    delete ui;
}

void ZcatthisControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZcatthisControlDialog::updateList()
{
...
void ZcatthisControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZcatthisControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZcatthis_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZcatthisControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZcatthisControlDialog::GetSelectedMints()
{
...
void ZcatthisControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
